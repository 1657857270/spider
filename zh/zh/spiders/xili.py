...
class XiliSpider(scrapy.Spider):
    name = 'xili'
    allowed_domains = ['www.zaih.com']
    page = 1
    url_format = 'https://www.zaih.com/falcon/mentors?first_tag_id=479&first_tag_name=心理&page={}'
    start_urls = [url_format.format(page)]

    def parse(self, response):
        # empty = response.css("section")
        # print(len(empty))
        mentors = response.xpath('//div[@class="mentor-board board"]/a')
        for m in mentors:
            item = ZhItem()
